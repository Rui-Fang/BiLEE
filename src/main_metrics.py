...
def validate_candidate_has_enough_ranking(qid_to_ranked_candidate_documents):
    for qid in qid_to_ranked_candidate_documents:
        if len(qid_to_ranked_candidate_documents[qid]) > MaxMRRRank:
            logger.info("Too many documents ranked. Please Provide top 100 documents for qid:{}".format(qid))
...
def load_candidate_from_stream(f):
    ...
    logger.info("Quantity of Documents ranked for each query is as expected. Evaluating")
...
def quality_checks_qids(qids_to_relevant_documentids, qids_to_ranked_candidate_documents):
    ...
    for qid in qids_to_ranked_candidate_documents:
        ...
        if len(duplicate_pids - set([0])) > 0:
            message = "Cannot rank a document multiple times for a single query. QID={qid}, PID={pid}".format(qid=qid, pid=list(duplicate_pids)[0])
            allowed = False
...
def load_exclude(path_to_exclude_folder):
    ...
    logger.info("{} excluded qids loaded".format(len(qids)))
    return qids


def recall(args, result=None, recall_num=None, log=True):  # TODO: adjust for NQ320k
    assert result is not None, "result is None"
    recall_num = args.recall_num if recall_num is None else recall_num
    q_gt, q_pred = {}, {}
    prev_q = ""
    recall_dict = {}
    if args.trivia:
        for [query, pred, gt, _] in result:
            if query != prev_q:  # wait, will the query duplicate ?
                q_pred[query] = pred.split(",")
                prev_q = query
            if query in q_gt:  ## Add all query-docid pairs into the set (for next phase)
    ...
    for i in recall_num:
        ...
        recall_dict[i] = recall_avg
        if log:
            logger.info(f"recall@{i}: {recall_avg}")

    return recall_dict


def MRR100(args, result=None, log=True):
    assert result is not None, "result is None"
    mrr_total = 0
    query_num = 0
    for [_, pred, gt, rank] in result:
    ...
    if log:
        logger.info(f"MRR{len(pred_list)}: {mrr}")
    return mrr

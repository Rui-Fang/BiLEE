...
class QFunc(object):
    ...
    def P2(self, x):
        ...
        if x <= self.nb:
            ...
            return self.nb * (1 - self.bias) / ((x - self.nb) ** 2 + self.nb * (1 - self.bias))
        else:
    ...
    def __call__(self, delta):
        ...
        if delta >= 1:
            ...
            right = int(scipy.stats.binom.ppf(1 - 2 * delta / np.exp(2), self.n, self.bias))
            ...
            for k_1 in range(search_right, search_left - 1, -1):
                ...
                if delta_b < delta and delta <= delta_u:
                    if k_1 == -1:
                        mu1 = self.nb + np.sqrt(self.nb * (1 - self.nb) * (1 - delta) / delta)
                    else:
                        k = k_1 + 1
                        pp = self.p(k)
                        ee = self.e(k)
...
def adaptive_bentkus_seq_mean_zero(X, delta, B, eta=1.1, power=1.1, std_upper_bound=None):
    """
...
def adaptive_bentkus_seq(Y, delta, L, U, eta=1.1, power=1.1):
    ...
    def g_function(n, nmax, delta_2_k):
        quantitle = scipy.stats.norm.ppf(1 - 2 * delta_2_k / np.exp(2))
        return np.sqrt(np.floor(nmax / 2.0)) * (U - L) * quantitle / (n * 2 * np.sqrt(2))
    ...
    for k in np.arange(max(kmax, 1)):
        ...
        if nmax >= nmin:
            ...
            for n in np.arange(nmin, min(nmax, N) + 1):
                ...
                if n <= 2 or n == nmin or Abar_min != Abar_min_prev or mu_up[n - 2] != mu_up[n - 3]:
                    q_up_func = QFunc(n=nmax, A=Abar_min, B=mu_up[n - 2] - L if n >= 2 else U - L)
                    Sn_low[n - 1] = -q_up_func(delta_1_k / 2.0)
                else:
                    Sn_low[n - 1] = Sn_low[n - 2]
                mu_up[n - 1] = min(Ymean[n - 1] - Sn_low[n - 1] / n, mu_up[n - 2] if n >= 2 else U)

                if n <= 2 or n == nmin or Abar_min != Abar_min_prev or mu_low[n - 2] != mu_low[n - 3]:
                    q_low_func = QFunc(n=nmax, A=Abar_min, B=U - mu_low[n - 2] if n >= 2 else U - L)
                    Sn_up[n - 1] = q_low_func(delta_1_k / 2.0)
                else:
                    Sn_up[n - 1] = Sn_up[n - 2]

                mu_low[n - 1] = max(Ymean[n - 1] - Sn_up[n - 1] / n, mu_low[n - 2] if n >= 2 else L)

...
wasserstein_ext = torch.utils.cpp_extension.load_inline(
    "wasserstein", cpp_sources="", cuda_sources=cuda_source, extra_cuda_cflags=["--expt-relaxed-constexpr"]
)
...
mu1 = torch.distributions.Normal(20.0, 10.0).log_prob(x).exp()
mu2 = torch.distributions.Normal(60.0, 30.0).log_prob(x).exp()
mu3 = torch.distributions.Normal(40.0, 20.0).log_prob(x).exp()
mu1 /= mu1.sum()
mu2 /= mu2.sum()
mu3 /= mu3.sum()

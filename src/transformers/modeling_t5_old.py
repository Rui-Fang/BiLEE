...
class T5Block(nn.Module):
    ...
    def forward(
        ...
        return (
            outputs  # hidden-states, present_key_value_states, (self-attention weights), (self-attention position bias), (cross-attention weights), (cross-attention position bias)
        )
...
class T5Model(T5PreTrainedModel):
    def __init__(self, config: T5Config):
        ...
        if self.multiple_decoder:  # QUESTION: Why mulitiple decoder? For model esemble?
            self.decoder_list = []
    ...
    def forward(
        ...
        **kwargs,
        ...
        #    )
        if only_encoder:  # QUESTION: So it can use encoder only?
...
class HierarchicT5Stack(T5PreTrainedModel):
    def __init__(self, config, embed_tokens=None, depth=1):
        ...
        self.stacks = nn.ModuleList([T5Stack(config, embed_tokens) for _ in range(depth)])
    ...
    def forward(
        ...
        if is_train:
            ...
            # logger.info(f"generate for {cur_depth}th token")
...
class T5ForConditionalGeneration(T5PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        if "Rdrop" in config.__dict__:  # 0.15 by default -M
            self.Rdrop = config.Rdrop
        if "Rdrop_only_decoder" in config.__dict__:  # 0 by default -M
            self.Rdrop_only_decoder = config.Rdrop_only_decoder
        if "Rdrop_loss" in config.__dict__:  # "KL" by default -M
            self.Rdrop_loss = config.Rdrop_loss
        if "embedding_distillation" in config.__dict__:  # 0.0 by default -M
            self.embedding_distillation = config.embedding_distillation
        if "weight_distillation" in config.__dict__:  # 0.0 by default -M
            self.weight_distillation = config.weight_distillation
        self.Rdrop_loss = "Contrast"  # What the HELL is this? -M

        self.shared = nn.Embedding(config.vocab_size, config.d_model)
        semantic_identifier = getattr(config, "semantic_identifier", None)  # 2 by default -M
        hierarchic_decode = getattr(config, "hierarchic_decode", None)  # 0 by default -M
        self.decode_vocab_size = getattr(config, "decode_vocab_size", None)  # 0 by default -M
        """
            if self.args.position(1 by default):
        ...
        """
        tie_decode_embedding = getattr(config, "tie_decode_embedding", None)  # 1 by default -M
        # decode_depth = getattr(config, "decode_depth", None)
        self.adaptor_decode = getattr(config, "adaptor_decode", None)  # 1 by default -M
        self.adaptor_efficient = getattr(config, "adaptor_efficient", None)  # 1 by default -M
        self.denoising = getattr(config, "denoising", None)  # 0 by default -M
        self.multiple_decoder = getattr(config, "multiple_decoder", None)  # 0 by default -M
        self.decoder_num = getattr(config, "decoder_num", None)  # 1 by default -M
        self.max_output_length = getattr(config, "max_output_length", None)  # 10 by default -M
        self.output_vocab_size = getattr(config, "output_vocab_size", None)  # 30 by default -M
        ...
        if self.multiple_decoder:
            ...
            logger.info("num_decoder_layers", config.num_decoder_layers)
        else:
            decoder_config.num_layers = config.num_decoder_layers
        if self.multiple_decoder:
            self.decoder_list = []
            ...
            if semantic_identifier and hierarchic_decode:
        ...
        logger.info("adaptor_decode", self.adaptor_decode)
        logger.info("adaptor_efficient", self.adaptor_efficient)
        ...
        if semantic_identifier:
            ...
            logger.info(bz, seq_length, vocab_size, output_vocab_size)
    ...
    def forward(
        ...
        **kwargs,
        ...
        # Decode
        # logger.info(f"decoder_input_ids: {decoder_input_ids}")
        # if past_key_values is not None:
        #     logger.info(f"past_key_values: {len(past_key_values)}")
        #     logger.info(f"past_key_values[0]: {len(past_key_values[0])}")
        #     logger.info(f"past_key_values[0][0].shape: {past_key_values[0][0].shape}")
        #     logger.info(f"past_key_values[0][1].shape: {past_key_values[0][1].shape}")
        # else:
        #     logger.info(f"past_key_values: None")

        # logger.info(f"decoder_attention_mask: {decoder_attention_mask}")
        var_list = [
            decoder_input_ids,
        ...
        if self.multiple_decoder:
            # logger.info(self.training, decoder_index)
        ...
        if self.adaptor_decode and not self.adaptor_efficient:
            ...
            ## TODO: add activation?
            # logger.info("adaptor_weight shape:{}".format(adaptor_weight.shape))
        ...
        """
            ...
            decoder_input_embedding = self_adaptor(decoder_input_embedding.transpose(0, 1), encode_embedding.transpose(0, 1), tgt_mask=mask).transpose(0, 1)
            adaptor_weight = self_adaptor_linear(decoder_input_embedding).reshape(decoder_input_embedding.shape[0], decoder_input_embedding.shape[1], self.model_dim, -1)
        ...
        def select_valid_embedding(sequence):  # QUESTION: what is that?
            bz = sequence.shape[0]
            ...
            # logger.info("mask", mask.shape, mask)
            sequence = sequence + mask
            # logger.info("result sequence", sequence.shape. sequence)
            return sequence
        ...
        if labels is not None:
            if self.Rdrop > 0 and self.training:
                ...
                if self.Rdrop_loss == "KL":
                    p_loss = F.kl_div(
                        F.log_softmax(lm_logits_half.view(-1, lm_logits.size(-1)), dim=-1), F.softmax(lm_logits_another.view(-1, lm_logits.size(-1)), dim=-1), reduction="none"
                    )
                    q_loss = F.kl_div(
                        F.log_softmax(lm_logits_another.view(-1, lm_logits.size(-1)), dim=-1), F.softmax(lm_logits_half.view(-1, lm_logits.size(-1)), dim=-1), reduction="none"
                    )
                    p_loss = p_loss.sum()
                    q_loss = q_loss.sum()
                    dist_loss = (p_loss + q_loss) / 2 / sl
                elif self.Rdrop_loss == "Contrast":  # Sounds the final version be forced to use this method. -M
                ...
                # logger.info("orig_loss", orig_loss)
            else:
                loss = loss_fct(lm_logits.view(-1, lm_logits.size(-1)), labels.view(-1))
                # logger.info("loss", loss)
            # logger.info(labels.shape, lm_logits.shape)
            ...
            if self.embedding_distillation > 0:
                ...
                end_token_emb = torch.cat([hidden_states[idx, i, :].unsqueeze(-2) for idx, i in enumerate(torch.where(input_ids == 1)[1])], dim=0)
            ...
            if self.weight_distillation > 0:
                ...
                _temp_weight = _temp_weight.reshape(_temp_weight.shape[0], _temp_weight.shape[1], _temp_weight.shape[1], -1, _temp_weight.shape[-1])

...
class T5ForConditionalGeneration(T5PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.shared = nn.Embedding(config.vocab_size, config.d_model)
        semantic_identifier = getattr(config, "semantic_identifier", None)
        ...
        if semantic_identifier:
            ...
            self.semantic_identifier = nn.Embedding(decode_vocab_size + 2, config.d_model)
        else:
            self.semantic_identifier = self.shared
        ...
        if hierarchic_decode:
            ...
            self.decoder = self.decoder = HierarchicT5Stack(decoder_config, self.semantic_identifier, depth=decode_depth)
        else:
            self.decoder = T5Stack(decoder_config, self.semantic_identifier)

        if semantic_identifier and decode_vocab_size:
            self.lm_head = nn.Linear(config.d_model, decode_vocab_size + 2, bias=False)
        else:
            self.lm_head = nn.Linear(config.d_model, config.vocab_size, bias=False)

        if tie_decode_embedding:
            self._tie_or_clone_weights(self.lm_head, self.semantic_identifier)

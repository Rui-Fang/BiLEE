        ...
        for ckpt_name in ckpt_files:
            ...
            if certain_epoch is not None:
                ...
                if ckpt_epoch > desired_ckpt_epoch:
                    desired_ckpt_epoch, desired_ckpt_name = ckpt_epoch, ckpt_name
    logger.info("=" * 20)
    logger.info("Loading: " + desired_ckpt_name)
    logger.info("=" * 20)
...
def load_data(args):
    def process_func(doc_to_query_list, index, query, newid, rank=1):
        if args.kary:  # Kary sounds like the hyperparameter in k-means
            docid = "-".join([c for c in str(newid).split("-")])  # Why?
            softmax_index_list = str(newid).split("-")[:3]
            softmax_index = 0
            for id, num in enumerate(softmax_index_list[::-1]):
                softmax_index += int(num) * (args.kary**id)  # can be faster but ok
        else:
    ...
    if "gtq" in args.query_type:
        if args.trivia:
            train_file = args.data_path + "TriviaQA/train.tsv"
            df = pd.read_csv(
                train_file,
                encoding="utf-8",
            ...
            train_filename = args.data_path + "NQ/nq_train_doc_newid.tsv"
            df = pd.read_csv(
                train_filename,
                names=["query", "queryid", "oldid", "bert_k30_c30_1", "bert_k30_c30_2", "bert_k30_c30_3", "bert_k30_c30_4", "bert_k30_c30_5"],
                encoding="utf-8",
        ...
        if "qg" in args.query_type:
            if args.trivia:
                qg_file = args.data_path + "TriviaQA/trivia_512_qg.tsv"
                gq_df1 = pd.read_csv(
                    qg_file,
                ...
                qg_file = args.data_path + "NQ/NQ_512_qg.tsv"
                gq_df1 = pd.read_csv(
                    qg_file,
                    names=["query", "queryid", "oldid", "bert_k30_c30_1", "bert_k30_c30_2", "bert_k30_c30_3", "bert_k30_c30_4", "bert_k30_c30_5"],
                    encoding="utf-8",
            ...
            result = tuple(process_func(doc_to_query_list, index, *row) for index, row in enumerate(zip(df["query"], df[args.id_class])))
            result_add1 = tuple(process_func(doc_to_query_list, index, *row) for index, row in enumerate(zip(gq_df1["query"], gq_df1[args.id_class])))
            result = result + result_add1
            logger.info("QG data loaded")
        else:
            result = tuple(process_func(doc_to_query_list, index, *row) for index, row in enumerate(zip(df["query"], df[args.id_class])))
    ...
    if "doc" in args.query_type:
        if args.trivia:
            filename = args.data_path + "TriviaQA/trivia_title_cont.tsv"
            path_list.append(filename)
        elif args.nq:
            filename = args.data_path + "NQ/nq_title_abs.tsv"
            path_list.append(filename)
        logger.info("doc loaded")

    if "doc_aug" in args.query_type:
        if args.trivia:
            filename = args.data_path + "TriviaQA/trivia_doc_aug.tsv"
            path_list.append(filename)
        elif args.nq:
            filename = args.data_path + "NQ/NQ_doc_aug.tsv"
            path_list.append(filename)
        logger.info("argumented doc loaded")
    ...
    for file_path in path_list:
        ...
        if args.trivia:
            df1 = pd.read_csv(
                ...
                sep="\t",
                dtype={"query": str, "queryid": str, "oldid": str, args.id_class: str},
            ).loc[:, ["query", "queryid", args.id_class]]
        elif args.nq:
            df1 = pd.read_csv(
                file_path,
                names=["query", "queryid", "oldid", "bert_k30_c30_1", "bert_k30_c30_2", "bert_k30_c30_3", "bert_k30_c30_4", "bert_k30_c30_5"],
        ...
        result_add1 = tuple(
            filter(None, (process_func(doc_to_query_list, index, *row) for index, row in enumerate(zip(df1["query"], df1[args.id_class]))))
        )  # TODO: add multi-process here
        result = result_add1 if result is None else result + result_add1
    logger.info("&" * 20)
    logger.info(result[0])
    logger.info("&" * 20)
    # torch.save(result,"processed_data.pth")
    logger.info("Dataloading finished...")
    return result, q_emb, query_id_dict_train, prefix_embedding, prefix_mask, prefix2idx_dict


def load_data_infer(args, phase="dev"):
    df = None
    if args.trivia:
        file_name = args.data_path + f"TriviaQA/{phase}.tsv"
        df = pd.read_csv(
            file_name,
            encoding="utf-8",
            names=["query", "queryid", "oldid", "bert_k30_c30_1", "bert_k30_c30_2", "bert_k30_c30_3", "bert_k30_c30_4", "bert_k30_c30_5"],
        ...
        dev_filename = args.data_path + "NQ/nq_dev_doc_newid.tsv"
        df = pd.read_csv(
            dev_filename,
            names=["query", "queryid", "oldid", "bert_k30_c30_1", "bert_k30_c30_2", "bert_k30_c30_3", "bert_k30_c30_4", "bert_k30_c30_5"],
            encoding="utf-8",
    ...
    for index, row in df.iterrows():
        ...
        rank = 1  # QUESTION : rank is always 1?

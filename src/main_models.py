...
from main_utils import assert_all_frozen, load_data_infer, load_data, numerical_decoder, dec_2d
...
class TreeBuilder(object):
    ...
    def add(self, seq) -> None:
        """
        seq is List[Int] representing id, without leading pad_token(hardcoded 0),
            with trailing eos_token(hardcoded 1) and (possible) pad_token, every int is token index
...
def decode_token(args, seqs):
    ...
    for seq in seqs:
        try:
            ...
            logger.info("no eos token found")
            logger.info(seq)
        if args.position:
                ...
                if self.args.kary:
                    ...
                    for i in range(0, len(target_id)):
                        ...
                        if self.args.position:
                            temp = i * self.args.output_vocab_size + int(c) + 2 if not self.args.hierarchic_decode else int(c) + 2
                        else:
                            temp = int(c) + 2
...
class T5FineTuner(pl.LightningModule):
    def _build_tree(self, args):
        ...
        if os.path.isfile(tree_save_path):
            ...
            builder = TreeBuilder()
            if args.trivia:
                trivia_names = ["query", "queryid", "oldid", "bert_k30_c30_1", "bert_k30_c30_2", "bert_k30_c30_3", "bert_k30_c30_4", "bert_k30_c30_5"]
                trivia_dtype = {"query": str, "queryid": str, "oldid": str, args.id_class: str}
                df_dicts = {
                    phase: pd.read_csv(
                        os.path.join(args.data_path, f"{phase}.tsv"), encoding="utf-8", names=trivia_names, header=None, sep="\t", dtype=trivia_dtype
    ...
    def __init__(self, args, train=True):
        ...
        t5_config = T5Config(
            ...
            embedding_distillation=args.embedding_distillation,  # Not a vanilla T5?
    ...
    def _step(self, batch):
        ...
        if self.args.multiple_decoder:
            encoder_outputs, input_mask, generation_loss, denoising_loss = self.forward(
    ...
    def _step_i(self, batch, i, encoder_outputs=None, input_mask=None):
        ...
        if self.args.hard_negative and self.args.sample_neg_num > 0:
            ...
            neg_outputs = self.forward(
                input_ids=sources_ids,
                decoder_index=i,
                encoder_outputs=outputs.encoder_outputs,
                attention_mask=attention_mask,
                lm_labels=neg_lm_labels,
        ...
        def cal_contrastive(outputs, neg_outputs):
            ...
            if neg_outputs is not None:
                decoder_embed = (
                    torch.cat((outputs.decoder_hidden_states[-1], neg_outputs.decoder_hidden_states[-1]), dim=0).transpose(0, 1).transpose(1, 2)
                )  # [seq_length, embed_size, batch_size*2]
            ...
            # logger.info("decoder_embed", decoder_embed.shape)  #[seq_length, embed_size, batch_size + neg_bz]
            # logger.info("decoder_weight", decoder_weight.shape) #[seq_length, embed_size, batch_size + neg_bz]
    ...
    def training_step(self, batch, batch_idx):
        ...
        # self.train_output_list.append(outputs) # memory leak
        return outputs
    ...
    def validation_step_i(self, batch, i):
        ...
        assert not self.args.softmax

        if self.args.semantic_identifier == 1:
        ...
        elif self.args.semantic_identifier == 2:
            if self.args.multiple_decoder:
    ...
    def on_validation_epoch_end(self):
        ...
        if self.args.multiple_decoder:
            ...
            for index in range(self.args.decoder_num):
                ...
                cur_inf_result_cache_prob = [
                    softmax(
                        sublist["inf_result_batch_prob"][
                            i
                            * int(len(sublist["inf_result_batch_prob"]) / len(outputs[index][0]["inf_result_batch"])) : (i + 1)
                            * int(len(sublist["inf_result_batch_prob"]) / len(outputs[index][0]["inf_result_batch"]))
                        ]
                    )
                    for sublist in outputs[index]
        ...
        total = 0
        for q in q_pred:
            # logger.info(q, q_pred[q], q_gt[q]) ## bunch of prints
            is_hit = 0
            for p in q_gt[q]:
                if p in q_pred[q]:
                    is_hit = 1
        ...
        print("")
        logger.info("{} recall@{}:{}".format(self.current_epoch, self.args.decoder_num, recall_avg))
    ...
    def train_dataloader(self):
        ...
        dataloader = DataLoader(
            train_dataset, sampler=sampler, batch_size=self.args.train_batch_size, drop_last=True, shuffle=False, num_workers=self.args.num_workers
        )
        return dataloader
    ...
    def val_dataloader(self):
        ...
        dataloader = DataLoader(
            val_dataset, sampler=sampler, batch_size=self.args.eval_batch_size, drop_last=True, shuffle=False, num_workers=self.args.num_workers
        )
        return dataloader
